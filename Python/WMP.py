...
def mapDraw2(m3,lons,lats,elons,elats,xy,exy,pv,vartype,areatype='full',colormap='viridis',SaveFig=True,picname=None):
    ...
    if pstreamlines:
        imd=streamplot(lons[::step,::step],lats[::step,::step],uReg[::step,::step]*constants.knotcoef,vReg[::step,::step]*constants.knotcoef,transform=ccrs.PlateCarree(),zorder=100,color='k',density=8.0,linewidth=0.20,maxlength=5.0,arrowsize=0.7) 
    ...
    plt.title(name+', '+units+', '+codes_get(gid,"typeOfLevel")+' '+ str(codes_get(gid,"level"))+'\n '+ CallesEgna.getDates(gid)[0].strftime("%Y-%m-%d %H:%M UTC")+', valid: '+CallesEgna.getDates(gid)[1].strftime("%Y-%m-%d %H:%M UTC")+' '+codes_get(gid,"stepType")+', mbr'+str(codes_get(gid,"perturbationNumber")),fontsize=11)
    #plt.show()
    if SaveFig:    #savePath+'MaxWind_'+Cycle+str(K).zfill(2)+'.png'
        if picname != None:
            ...
            savefig(savePath+picname + '_' + CallesEgna.getDates(gid)[0].strftime("%H").zfill(2)+str(codes_get(gid,"step")).zfill(2) + '.png',bbox_inches='tight',pad_inches=0.4,dpi=70)
        else:
            savefig(savePath+codes_get(gid,"shortName")+str(codes_get(gid,"step"))+'_'+codes_get(gid,"stepType")+'_'+codes_get(gid,"typeOfLevel")+str(codes_get(gid,"level"))+'_mbr'+str(codes_get(gid,"perturbationNumber"))+'.png',bbox_inches='tight',pad_inches=0.4)
        plt.close()
...
#======================================
            ...
            if 'grib2_fp_mbr' in infile:
                ...
                rain_accum,gid = getPV('rain','heightAboveGround',0,'any','accum');codes_release(gid)
                snow_accum,gid = getPV('snow','heightAboveGround',0,'any','accum');codes_release(gid)
                grpl_accum,gid = getPV('grpl','heightAboveGround',0,'any','accum')
                ...
                # CLOUD DEPTH
                pv = ct - cb
                mapDraw2(m3,lons,lats,elons,elats,xy,exy,cbf,'cloud_depth','full',picname='CloudDepth')
                codes_release(gid)
