...
def metGrams(location):
    ...
    fig.suptitle(location.name + f' (Lon: {location.longitude:.2f}, Lat: {location.latitude:.2f}), '+location.initial[0][0].strftime("%Y-%m-%d Cycle %HZ")+", MEPS member 000",size=11,y=0.99)
    ...
    ##### PANEL 5 DEFINITIONS -- Near surface temperatures and wind
    par5 = axarr[4].twinx()
    ...
    for subplt in range(subplts):
        ...
        axarr[subplt].xaxis.set_minor_locator(mdates.HourLocator(interval=3))
        axarr[subplt].tick_params(axis='x', which='major',pad=10,labelsize=7)
